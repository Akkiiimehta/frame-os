...
const SC = {
  ...
  Pending: {
    dot: 'var(--amber)',
    bg: 'var(--amber-bg)',
    bd: 'rgba(255,214,10,.2)',
  },
  Partial: {
    dot: 'var(--orange)',
    bg: 'var(--orange-bg)',
    bd: 'rgba(255,159,10,.2)',
  },
  Paid: {
    dot: 'var(--green)',
    bg: 'var(--green-bg)',
...
const BRAND_GRADS = [
  'linear-gradient(135deg,#1a2f6e,#2a4fa8)',
  'linear-gradient(135deg,#1e3a2f,#2d6b50)',
  'linear-gradient(135deg,#3a1a2e,#7a2d5a)',
  'linear-gradient(135deg,#2a1a10,#7a4010)',
  'linear-gradient(135deg,#0a2240,#1a5080)',
  'linear-gradient(135deg,#2a0a3a,#6020a0)',
];
const AVC = [
  '#2563eb',
  '#7c3aed',
...
const SEED_CREW = [
  {
    ...
    role: 'DOP',
    phone: '9876543210',
    email: 'kabir@frameops.in',
    location: 'Mumbai',
    tags: ['arri', 'reliable'],
    notes: 'Prefers natural light.',
    projects: [1, 2],
  },
  {
    id: 102,
    ...
    role: 'Producer',
    phone: '9988776655',
    email: 'riya@frameops.in',
    location: 'Mumbai',
    tags: ['logistics'],
    notes: 'Strong vendor network.',
    projects: [1, 3],
  },
  {
    id: 103,
    ...
    location: 'Delhi',
    tags: ['commercial'],
    notes: 'Known for fashion campaigns.',
    projects: [2, 4],
  },
  {
    id: 104,
  ...
  },
  {
    ...
    role: 'Gaffer',
    phone: '9876001234',
    email: 'zara.g@lights.in',
    location: 'Mumbai',
    tags: ['studio'],
    notes: 'Full lighting kit.',
    projects: [1],
  },
  {
    id: 106,
    ...
    role: 'AD',
    phone: '9001234567',
    email: 'dev.ad@film.in',
    location: 'Mumbai',
    tags: ['organised'],
    notes: 'Excellent with large shoots.',
    projects: [2],
  },
];
const SEED_PROJECTS = [
  {
    ...
    crewIds: [101, 102, 105],
    tags: ['fashion', 'TVC'],
    notes: 'Second day wrapping up.',
  },
  {
    id: 2,
    title: 'Taj Hotels Brand Film',
    client: 'Taj Group',
    type: 'Brand Film',
...
const SEED_INV = [
  {
    ...
    project: 'Zara Campaign TVC',
    client: 'Zara India',
    amount: 160000,
    status: 'Partial',
    due: '2026-04-01',
    payments: [
      { id: 101, amount: 80000, date: '2026-03-10', note: '50% advance' },
    ],
  },
  {
    id: 2,
    invoiceNo: 'INV-2026-002',
    project: 'Taj Hotels Brand Film',
    client: 'Taj Group',
    amount: 290000,
    status: 'Paid',
    due: '2026-03-15',
    payments: [
      { id: 201, amount: 145000, date: '2026-03-01', note: 'First instalment' },
      { id: 202, amount: 145000, date: '2026-03-14', note: 'Final payment' },
    ],
  },
  {
    id: 3,
    invoiceNo: 'INV-2026-003',
    project: 'Mamaearth Product Shoot',
    client: 'Mamaearth',
    amount: 95000,
    status: 'Overdue',
    due: '2026-03-10',
...
const autoSt = (inv) => {
  const r = totalRec(inv);
  if (r === 0) return inv.status === 'Overdue' ? 'Overdue' : 'Pending';
  if (r >= inv.amount) return 'Paid';
  return 'Partial';
};
const rc = (role) => {
  const m = {
...
function SC2({ label, value, sub, color, icon, delay = 0 }) {
  return (
    <div
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 26, fontWeight: 600, color, lineHeight: 1 }}>
            {value}
          </div>
          {sub && (
        ...
        <span style={{ fontSize: 20, opacity: 0.35, marginTop: 2 }}>
          {icon}
        </span>
      </div>
    </div>
...
function ETags({ tags, onAdd, onDelete }) {
...
function ENotes({ notes, onSave }) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState(notes || '');
  if (editing)
    return (
      <div>
        <TA value={draft} onChange={setDraft} placeholder="Add notes…" />
...
function StCell({ status, onChange }) {
  ...
  return (
    <div ref={ref} style={{ padding: '10px 16px', position: 'relative' }}>
      <div
        onClick={() => setOpen((o) => !o)}
        style={{
          display: 'inline-flex',
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 7,
                  cursor: 'pointer',
                  background: act ? 'var(--bg4)' : 'transparent',
                }}
                onMouseEnter={(e) =>
              ...
              >
                ...
                {act && (
                  <span
                    style={{ marginLeft: 'auto', fontSize: 11, color: ss.dot }}
                  >
                    ✓
                  </span>
                )}
...
function genCS(cs, crewMap) {
  ...
  const crewBlock = keyRoles
    .map((role) => {
      const c = crew.find((m) => m.role === role);
      return c ? `<div class="ci"><b>${role}:</b> ${c.name}</div>` : '';
    })
    .filter(Boolean)
    .join('');
  const deptRows = (() => {
    const d = cs.deptCalls || [];
    const rows = [];
    for (let i = 0; i < d.length; i += 2) {
      rows.push(
        `<tr><td>${d[i].dept}</td><td>${d[i].time || 'OC'}</td><td>${
          d[i + 1] ? d[i + 1].dept : ''
        }</td><td>${d[i + 1] ? d[i + 1].time || 'OC' : ''}</td></tr>`
      );
    }
    return rows.join('');
  })();
  const castRows = (cs.cast || [])
    .map(
      (c) =>
        `<tr><td>${c.no}</td><td><b>${c.name}</b></td><td>${c.number}</td><td>${
          c.reach
        }</td><td>${c.hmw}</td><td>${c.onset}</td><td>${
          c.notes || '—'
        }</td></tr>`
    )
    .join('');
  const shotRows = (cs.schedule || [])
    .map(
      (s, i) =>
        `<tr style="${i % 2 === 0 ? 'background:#fafafa' : ''}"><td>${
          s.no
        }</td><td><b>${s.scene}</b></td><td>${s.description}</td><td>${
          s.characters
        }</td><td>${s.costumes}</td><td><b>${s.eta}</b></td></tr>`
    )
    .join('');
  const prod = crew.find(
    (m) => m.role === 'Producer' || m.role === 'Executive Producer'
  );
  const ad = crew.find((m) => m.role === 'AD' || m.role === '1st AD');
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Call Sheet — ${
    cs.projectTitle || 'Shoot'
  }</title><style>
  *{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;background:#fff;color:#000;font-size:12px;}
  ...
  <div class="hbox">
    <div class="htop">
      <div class="logo-c"><div class="logo-init">${(cs.productionHouse || 'FO')
        .substring(0, 2)
        .toUpperCase()}</div><div class="logo-sub">${
    cs.productionHouse || 'FRAME OS'
  }</div></div>
      <div class="shoot-c"><div style="font-size:14px;font-weight:700;">SHOOT DATE: ${
        cs.shootDate || '—'
      }</div><div style="margin-top:4px;font-size:12px;">Shooting Day: ${
    cs.shootDay || '1/1'
  }</div><div style="margin-top:8px;font-weight:700;font-size:13px;">CREW CALL — ${
    cs.crewCall || '6:00 AM'
  }</div><div>SHIFT: ${cs.shiftStart || '7:00 AM'} — ${
    cs.shiftEnd || '7:00 PM'
  }</div></div>
      <div class="loc-c"><div style="font-weight:700;font-size:10px;margin-bottom:4px;">LOCATION:</div><div>${(
        cs.location || ''
      ).replace(
        /\n/g,
        '<br>'
      )}</div><div style="font-weight:700;font-size:10px;margin:8px 0 3px;">CONTACT ON SET:</div><div>${
    contactStr || cs.contactOnSet || '—'
  }</div></div>
    </div>
    <div class="info-g">
      <div class="ic"><div class="lbl">MEALS</div>Breakfast: ${
        cs.breakfast || '7:00 AM onwards'
      }<br>Lunch: ${cs.lunch || '1:00 PM'}<br>Evening Snacks: ${
    cs.snacks || '5:30 PM'
  }</div>
      <div class="ic"><div class="lbl">WEATHER</div>${
        cs.weather || 'Clear sky'
      } · ${cs.temp || '—'}<br>Sunrise: ${cs.sunrise || '—'} · Sunset: ${
    cs.sunset || '—'
  }<br><b>Nearest Hospital:</b> ${cs.hospital || '—'}</div>
    </div>
    <div class="crew-s"><div class="cg">${
      crewBlock ||
      crew
        .map((c) => `<div class="ci"><b>${c.role}:</b> ${c.name}</div>`)
        .join('')
    }</div></div>
  </div>
  <div class="dec">SHOOT DECORUM: ${
    cs.decorum ||
    'NO PERSONAL PHOTOGRAPHY ON SET · NO OPEN SHOES · NO SMOKING · DO NOT TOUCH PROPS · MAINTAIN SILENCE'
  }</div>
  ${
    (cs.deptCalls || []).length > 0
      ? `<div class="sh">CALL TIME ON SET</div><table><thead><tr><th>DEPARTMENT</th><th>CALL TIME</th><th>DEPARTMENT</th><th>CALL TIME</th></tr></thead><tbody>${deptRows}</tbody></table>`
      : ''
  }
  ${
    (cs.cast || []).filter((c) => c.name).length > 0
      ? `<div class="sh">PRIMARY CAST</div><table><thead><tr><th>SR.NO</th><th>CAST</th><th>NUMBER</th><th>REACH</th><th>HMW</th><th>ON SET</th><th>NOTES</th></tr></thead><tbody>${castRows}</tbody></table>`
      : ''
  }
  ${
    (cs.schedule || []).filter((s) => s.scene).length > 0
      ? `<div class="sh">SHOOTING SCHEDULE</div><table><thead><tr><th>SR NO</th><th>SCENE</th><th>DESCRIPTION</th><th>CHARACTERS</th><th>COSTUMES</th><th>ETA</th></tr></thead><tbody>${shotRows}</tbody></table>`
      : ''
  }
  ${
    cs.requirements
      ? `<div class="sh">REQUIREMENTS</div><div style="padding:10px 14px;border:2px solid #000;border-top:none;font-size:11px;white-space:pre-wrap">${cs.requirements}</div>`
      : ''
  }
  <div class="appr"><div class="ac"><b>Approved by Executive Producer:</b> ${
    prod ? prod.name : '_______________________'
  }</div><div class="ac"><b>1st Assistant Director:</b> ${
    ad ? ad.name : '_______________________'
  }</div></div>
...
function CSPanel({ project, allCrew, onClose }) {
  ...
  const defDepts = [
    { dept: 'Director', time: 'OC' },
    { dept: 'Camera Team', time: '8:00 AM' },
    { dept: 'DOP', time: 'OC' },
    { dept: 'Costume Team', time: '8:00 AM' },
    { dept: 'Producer', time: 'OC' },
    { dept: 'Art Team', time: '6:00 AM' },
    { dept: 'Client & Agency', time: '8:30 AM' },
    { dept: 'Sound Team', time: 'N/A' },
    { dept: 'Direction Team', time: '7:00 AM' },
    { dept: 'Make Up Team', time: '8:00 AM' },
    { dept: 'Food Stylist', time: '7:00 AM' },
    { dept: 'Production Support', time: '6:00 AM' },
    { dept: 'Lighting Team', time: '6:00 AM' },
    { dept: 'DIT', time: '8:30 AM' },
  ];
  const [cs, setCs] = useState({
    projectTitle: project.title,
    client: project.client,
    productionHouse: 'Frame OS',
    shootDate: project.shoot || '',
    shootDay: '1/1',
    crewCall: '6:00 AM',
    shiftStart: '7:00 AM',
    shiftEnd: '7:00 PM',
    ...
    hospital: '',
    decorum:
      'NO PERSONAL PHOTOGRAPHY ON SET · NO OPEN SHOES ON SET · NO SMOKING ON SET · DO NOT TOUCH PROPS ON SET · MAINTAIN SILENCE',
    crewIds: project.crewIds || [],
    deptCalls: defDepts,
    cast: [
      { no: 1, name: '', number: '', reach: '', hmw: '', onset: '', notes: '' },
    ],
    schedule: [
      {
        no: 1,
  ...
  const dl = () => {
    const html = genCS(cs, crewMap);
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([html], { type: 'text/html' }));
    a.download = `CallSheet_${cs.projectTitle.replace(/\s+/g, '_')}.html`;
    a.click();
  };
  ...
  return (
    <>
      <div className="cs-ovl" onClick={onClose} />
      <div className="cs-panel">
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn-p" onClick={dl}>
                ↓ Download
              </button>
              <button
                onClick={onClose}
        ...
        {tab === 'info' && (
          <div className="ps">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              >
                <div>
                  <Lbl ch="Production house" />
                  <Inp
                    value={cs.productionHouse}
                    onChange={(v) => upd('productionHouse', v)}
                    placeholder="Frame OS Productions"
                  />
                </div>
                <div>
                  <Lbl ch="Shoot date" />
              ...
              >
                <div>
                  ...
                  <Inp
                    value={cs.shiftEnd}
                    onChange={(v) => upd('shiftEnd', v)}
                    placeholder="7:00 PM"
                  />
                </div>
                <div>
                  <Lbl ch="Contact override" />
                  <Inp
                    value={cs.contactOnSet}
                    onChange={(v) => upd('contactOnSet', v)}
                    placeholder="Auto from crew"
                  />
                </div>
              </div>
              <div>
              ...
              >
                <div>
                  ...
                  <Inp
                    value={cs.breakfast}
                    onChange={(v) => upd('breakfast', v)}
                  />
                </div>
                <div>
                  <Lbl ch="Lunch" />
                  <Inp value={cs.lunch} onChange={(v) => upd('lunch', v)} />
                </div>
                <div>
                  <Lbl ch="Snacks" />
                  <Inp value={cs.snacks} onChange={(v) => upd('snacks', v)} />
                </div>
              </div>
              <div
                style={{
              ...
              >
                <div>
                  <Lbl ch="Weather" />
                  <Inp value={cs.weather} onChange={(v) => upd('weather', v)} />
                </div>
                <div>
                  <Lbl ch="Temp" />
                  <Inp value={cs.temp} onChange={(v) => upd('temp', v)} />
                </div>
                <div>
                  <Lbl ch="Sunrise" />
                  <Inp value={cs.sunrise} onChange={(v) => upd('sunrise', v)} />
                </div>
                <div>
                  <Lbl ch="Sunset" />
                  <Inp value={cs.sunset} onChange={(v) => upd('sunset', v)} />
                </div>
              </div>
              <div>
                <Lbl ch="Nearest hospital" />
                <Inp
                  value={cs.hospital}
              ...
              <div>
                <Lbl ch="Shoot decorum" />
                <TA value={cs.decorum} onChange={(v) => upd('decorum', v)} />
              </div>
            </div>
          </div>
        )}
        {tab === 'crew' && (
          <div className="ps">
            ...
            >
              Auto-filled from project crew. Edit call times below.
            </div>
            {crew.map((c, i) => (
              <div
                key={c.id}
            ...
            <button
              ...
              onClick={() =>
                upd('deptCalls', [...cs.deptCalls, { dept: '', time: '' }])
              }
            >
              + Add department
            </button>
        ...
        {tab === 'requirements' && (
          <div className="ps">
            <Lbl ch="Requirements (art, F&B, costume, lighting etc)" />
            <TA
              value={cs.requirements}
              onChange={(v) => upd('requirements', v)}
              placeholder={
                'ART:\n- Stand for product\n\nF&B:\n- Lettuce 2kg\n\nCOSTUME:\n- Smart casuals'
              }
            />
...
function InvPanel({ invoice, onClose, onUpdate }) {
  ...
  const rem = invoice.amount - rec;
  const pct = Math.min(
    100,
    invoice.amount > 0 ? (rec / invoice.amount) * 100 : 0
  );
  ...
  const addP = () => {
    ...
    const np = [
      ...invoice.payments,
      { id: Date.now(), amount: a, date: pf.date, note: pf.note },
    ];
    onUpdate(invoice.id, {
      payments: np,
      status: autoSt({ ...invoice, payments: np }),
    });
    setPf({ amount: '', date: '', note: '' });
  };
  const delP = (pid) => {
    const np = invoice.payments.filter((p) => p.id !== pid);
    onUpdate(invoice.id, {
      payments: np,
      status: autoSt({ ...invoice, payments: np }),
    });
    ...
    <>
      ...
      <div className="panel">
        ...
        <div className="ps">
          ...
          >
            {[
              ['Total', fmt(invoice.amount), 'var(--text)'],
              ['Received', fmt(rec), 'var(--green)'],
              [
                'Outstanding',
                fmt(Math.max(0, rem)),
                rem > 0 ? 'var(--red)' : 'var(--text3)',
              ],
            ].map(([l, v, c]) => (
              <div
                key={l}
            ...
            invoice.payments.map((p, i) => (
              <div
                ...
                style={{
                  ...
                  alignItems: 'flex-start',
                  gap: 12,
                  paddingBottom: i < invoice.payments.length - 1 ? 14 : 0,
                  marginBottom: i < invoice.payments.length - 1 ? 14 : 0,
                  borderBottom:
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              >
                <div>
                  ...
                  <Inp
                    value={pf.amount}
                    onChange={(v) => setPf((f) => ({ ...f, amount: v }))}
                    placeholder={`Max ${fmt(rem)}`}
                  />
                </div>
                <div>
                  <Lbl ch="Date" />
...
function ProjPanel({
  ...
  const sc2 = SC[project.status] || SC['On Hold'];
  const crewOnP = allCrew.filter((c) => project.crewIds.includes(c.id));
  ...
  const prog = pm[project.status] || 0;
  // Expense data from tracker
  const expEvents = () => {
    try {
      const d = JSON.parse(
        localStorage.getItem('ym_expense_pro_final') || '{}'
      );
  ...
  const matchedExp = expEvents().find(
    (ev) =>
      (ev.brand || '').toLowerCase() === project.client.toLowerCase() ||
      (ev.title || '')
        .toLowerCase()
        .includes(project.title.toLowerCase().split(' ')[0])
  );
  const expSpent = matchedExp
    ? (matchedExp.rows || []).reduce((s, r) => s + (parseFloat(r.amt) || 0), 0)
    : null;
  function CPicker() {
    const [open, setOpen] = useState(false);
    ...
    const avail = allCrew.filter(
      (c) =>
        !project.crewIds.includes(c.id) &&
          ...
          >
            ...
            {avail.map((c) => (
              <div
                key={c.id}
                onClick={() => {
                  onAddCrew(project.id, c.id, false);
                  setOpen(false);
                  setSearch('');
                }}
                style={{
              ...
              >
                <Av name={c.name} idx={allCrew.indexOf(c)} size={28} />
                <div>
                  <div
                    style={{
                      fontSize: 13,
      ...
      <div className="panel" style={{ width: 490 }}>
              ...
              {isAdmin && (
                <button
                  ...
                  style={{
                    ...
                    padding: '5px 12px',
                    color: 'var(--teal)',
                    borderColor: 'rgba(90,200,250,.25)',
                  }}
        ...
        <div className="ps">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            {['Pre-Prod', 'Shoot', 'Post', 'Delivered'].map((s, i) => {
              const done = prog >= [15, 45, 75, 100][i];
              return (
                <div
                  key={s}
                  style={{
                ...
                >
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: '50%',
                      background: done ? sc2.dot : 'var(--bg4)',
                      border: `1px solid ${done ? sc2.dot : 'var(--border2)'}`,
                    }}
                  />
                  <span
                    style={{
        ...
        <div className="ps">
          ...
          {[
            ['Shoot date', project.shoot || 'TBD'],
            ['Budget', isAdmin ? fmt(project.budget) : 'Hidden'],
            ['Type', project.type],
          ].map(([k, v]) => (
            <div key={k} className="drow">
              ...
              <span
                style={{
                  ...
                  color: 'var(--text)',
                  fontFamily:
                    k === 'Budget' ? "'DM Mono',monospace" : 'inherit',
                }}
              >
                {v}
              </span>
          ...
          <div style={{ paddingTop: 10 }}>
            ...
            {isAdmin ? (
              <Inp
                ...
                onChange={(v) => onUpdate(project.id, { location: v })}
                placeholder="Shoot location"
              />
            ) : (
              <div
                style={{ fontSize: 13, color: 'var(--text2)', paddingTop: 2 }}
              >
                {project.location || '—'}
              </div>
            )}
        ...
        {isAdmin && (
          <div className="ps">
            ...
            {matchedExp && expSpent !== null ? (
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      MATCHED EVENT: {matchedExp.title}
                    </div>
                    <div
                      style={{
                        fontSize: 16,
                  ...
                  <div
                    style={{
                      ...
                      width: `${Math.min(
                        100,
                        project.budget > 0
                          ? (expSpent / project.budget) * 100
                          : 0
                      )}%`,
                      background:
                        expSpent > project.budget
                          ? 'var(--red)'
                          : 'var(--amber)',
                      borderRadius: 5,
                      transition: 'width .5s',
                ...
                >
                  {fmt(project.budget - expSpent)} remaining ·{' '}
                  {((expSpent / project.budget) * 100).toFixed(0)}% of budget
                  used
                </div>
                <div
                  style={{ fontSize: 11, color: 'var(--text3)', marginTop: 4 }}
                >
                  Remaining: {fmt(Math.max(0, project.budget - expSpent))} ·
                  Burn: {((expSpent / project.budget) * 100).toFixed(0)}%
                </div>
              </div>
            ) : (
              <div
              ...
              >
                No matched expense event found for "{project.client}".
              </div>
            )}
            {expTrackerUrl && (
              <a
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      transition: 'all .15s',
                      background: act ? ss.bg : 'transparent',
                      color: act ? ss.dot : 'var(--text2)',
                      border: `1px solid ${act ? ss.bd : 'var(--border)'}`,
                      fontWeight: act ? 500 : 400,
                    }}
                  >
                    {s}
        ...
        <div className="ps">
          ...
          {isAdmin ? (
            <ETags
              tags={project.tags || []}
              onAdd={(t) =>
                onUpdate(project.id, { tags: [...(project.tags || []), t] })
              }
              onDelete={(t) =>
                onUpdate(project.id, {
        ...
        <div className="ps">
          ...
          {crewOnP.length === 0 ? (
            ...
            crewOnP.map((c) => (
              ...
              >
                <Av name={c.name} idx={allCrew.indexOf(c)} size={34} />
                ...
                {isAdmin && (
                  <button
                    onClick={() => onRemoveCrew(project.id, c.id)}
                    style={{
                      background: 'none',
                      border: 'none',
                      color: 'var(--text3)',
            ...
            >
              ...
              {totalRec(relInv) > 0 && (
                <div style={{ marginTop: 10 }}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        width: `${Math.min(
                          100,
                          (totalRec(relInv) / relInv.amount) * 100
                        )}%`,
                        background: 'var(--green)',
                        borderRadius: 3,
                      }}
                  ...
                  >
                    {fmt(totalRec(relInv))} of {fmt(relInv.amount)} received
                  </div>
                </div>
              )}
            </div>
...
function ProjectsView({ invoices, allCrew, setAllCrew, role, expTrackerUrl }) {
  ...
  const addCrew = (pid, cid, isNew) => {
    if (isNew) {
      const nm = { ...cid, projects: [pid] };
      setAllCrew((c) => [...c, nm]);
      setProjects((ps) =>
        ps.map((p) =>
          p.id === pid ? { ...p, crewIds: [...p.crewIds, nm.id] } : p
        )
      );
    } else {
      setProjects((ps) =>
        ps.map((p) =>
          p.id === pid ? { ...p, crewIds: [...p.crewIds, cid] } : p
        )
      );
      setAllCrew((c) =>
        c.map((m) =>
          m.id === cid ? { ...m, projects: [...m.projects, pid] } : m
        )
      );
    }
  };
  const remCrew = (pid, cid) => {
    setProjects((ps) =>
      ps.map((p) =>
        p.id === pid ? { ...p, crewIds: p.crewIds.filter((x) => x !== cid) } : p
      )
    );
    setAllCrew((c) =>
      c.map((m) =>
  ...
  return (
    <div>
      ...
      >
        ↗ tap card to view · {isAdmin && '📋 call sheet inside panel'}
      </div>
      <div
      ...
      >
        {shown.map((p, i) => {
          const sc2 = SC[p.status] || SC['On Hold'];
            ...
            >
              <div
                style={{
                  ...
                  right: 0,
                  height: 2,
                  background: `linear-gradient(90deg,transparent,${sc2.dot}55,transparent)`,
                }}
              />
              <div
                style={{
              ...
              >
                ...
                <div style={{ display: 'flex' }}>
                  {crew.slice(0, 4).map((c, j) => (
                    <div
                      key={c.id}
                      style={{ marginLeft: j > 0 ? -7 : 0, zIndex: 10 - j }}
                    >
                      <Av name={c.name} idx={allCrew.indexOf(c)} size={24} />
                    </div>
                  ))}
                  {crew.length > 4 && (
                    <div
      ...
      {selP && (
        <ProjPanel
          ...
          onUpdate={upd}
          onStatusChange={(id, s) => upd(id, { status: s })}
          onAddCrew={addCrew}
          onRemoveCrew={remCrew}
          role={role}
          expTrackerUrl={expTrackerUrl}
        />
      )}
      {showAdd && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
            <div>
              ...
              <Inp
                value={form.title}
                onChange={fset('title')}
                placeholder="Nike AW26 Campaign"
              />
            </div>
            <div>
              <Lbl ch="Client" />
            ...
            >
              <div>
                <Lbl ch="Shoot date" />
                <Inp type="date" value={form.shoot} onChange={fset('shoot')} />
              </div>
              <div>
                <Lbl ch="Budget (₹)" />
                <Inp
                  value={form.budget}
                  onChange={fset('budget')}
                  placeholder="250000"
                />
              </div>
            </div>
            <div>
              ...
              <Inp
                value={form.location}
                onChange={fset('location')}
                placeholder="Mumbai Studio, Bandra"
              />
            </div>
            <div>
              <Lbl ch="Drive URL" />
            ...
            >
              <button className="btn-g" onClick={() => setShowAdd(false)}>
                Cancel
              </button>
              <button className="btn-p" onClick={doAdd}>
                Add project
              </button>
            </div>
          </div>
...
function FinanceView() {
  ...
  const partial = invoices
    ...
    .reduce((s, i) => s + totalRec(i), 0);
  const pending = invoices
    .filter((i) => ['Pending', 'Partial'].includes(i.status))
    .reduce((s, i) => s + (i.amount - totalRec(i)), 0);
  ...
  const doAdd = () => {
    ...
    setInvoices((inv) => [
      ...inv,
      { ...form, id: Date.now(), amount: Number(form.amount), payments: [] },
    ]);
    setForm({
      invoiceNo: '',
      project: '',
  ...
  const expCSV = () => {
    const rows = [
      ['Invoice No', 'Project', 'Client', 'Total', 'Received', 'Status', 'Due'],
      ...invoices.map((i) => [
        i.invoiceNo,
        i.project,
        i.client,
    ...
    <div>
      ...
      >
        ...
        <SC2
          ...
          value={fmtK(pending)}
          sub={`${
            invoices.filter((i) => ['Pending', 'Partial'].includes(i.status))
              .length
          }`}
          color="var(--amber)"
          icon="⏳"
      ...
      <div className="card" style={{ padding: '16px 20px', marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
          {[
            ['Collected', 'var(--green)', paid + partial],
            ['Outstanding', 'var(--amber)', pending],
      ...
      >
        ✎ click amount · click badge · click row to open
      </div>
      <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
        <button className="btn-p" onClick={() => setShowAdd(true)}>
          + New invoice
        </button>
        <button className="btn-g" onClick={expCSV}>
          ↓ Export CSV
        </button>
      </div>
      <div className="fscroll">
        <div className="card" style={{ overflow: 'visible' }}>
          <div
            style={{
              display: 'grid',
              gridTemplateColumns: '1.1fr 2fr 1fr 1.4fr 1fr 1.4fr',
              background: 'var(--bg3)',
              borderBottom: '1px solid var(--border)',
          ...
          >
            {['Invoice', 'Project', 'Client', 'Amount', 'Due', 'Status'].map(
              (h) => (
                <div
                  key={h}
                  style={{
          ...
          {invoices.map((inv, i) => {
            ...
            return (
              <div
                ...
                onClick={(e) => {
                  if (!e.target.closest('[data-nc]')) setSelected(inv.id);
                }}
              >
                <div
                  style={{
                ...
                >
                  <ACell
                    value={inv.amount}
                    onChange={(v) => upd(inv.id, { amount: v })}
                  />
                  {rec > 0 && rec < inv.amount && (
                ...
                <div data-nc="1" onClick={(e) => e.stopPropagation()}>
                  <StCell
                    status={inv.status}
                    onChange={(st) => upd(inv.id, { status: st })}
                  />
                </div>
              </div>
            );
      ...
      {showAdd && (
        <Modal title="New invoice" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
            <div>
              <Lbl ch="Invoice number" />
              <Inp
                value={form.invoiceNo}
                onChange={fset('invoiceNo')}
            ...
            >
              <div>
                ...
                <Inp
                  value={form.amount}
                  onChange={fset('amount')}
                  placeholder="150000"
                />
              </div>
              <div>
                <Lbl ch="Due date" />
                <Inp type="date" value={form.due} onChange={fset('due')} />
              </div>
            </div>
            <div>
              <Lbl ch="Status" />
...
function ClientsView({ role }) {
  ...
  const names = [
    ...new Set([
      ...
      ...invoices.map((i) => i.client),
    ]),
  ];
  const clients = names.map((name, idx) => {
    const cP = projects.filter((p) => p.client === name);
    const cI = invoices.filter((i) => i.client === name);
    const tb = cI.reduce((s, i) => s + i.amount, 0);
    const tr = cI.reduce((s, i) => s + totalRec(i), 0);
    return {
      name,
      projects: cP,
      invoices: cI,
      ...
      totalReceived: tr,
      outstanding: tb - tr,
      hasOverdue: cI.some((i) => i.status === 'Overdue'),
      hasPending: cI.some((i) => ['Pending', 'Partial'].includes(i.status)),
      isRegular: cP.length >= 2,
      idx,
    };
  });
  ...
  return (
    <div>
      ...
      >
        ...
        <SC2
          label="Regular"
          value={clients.filter((c) => c.isRegular).length}
          color="var(--accent)"
          icon="⭐"
          delay={50}
        />
                ...
                >
                  <div
                    style={{
                      ...
                      height: 44,
                      borderRadius: 11,
                      background: BRAND_GRADS[i % BRAND_GRADS.length],
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 600,
                        color:
                          c.outstanding > 0 ? 'var(--red)' : 'var(--text3)',
                        fontFamily: "'DM Mono',monospace",
                      }}
                    >
                      {c.outstanding > 0 ? fmtK(c.outstanding) : '—'}
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="panel">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div
                    style={{
                      ...
                      height: 52,
                      borderRadius: 14,
                      background: BRAND_GRADS[sel.idx % BRAND_GRADS.length],
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
            ...
            {isAdmin && (
              <div className="ps">
                ...
                >
                  {[
                    ['Total billed', fmt(sel.totalBilled), 'var(--text)'],
                    ['Received', fmt(sel.totalReceived), 'var(--green)'],
                    [
                      'Outstanding',
                      fmt(sel.outstanding),
                      sel.outstanding > 0 ? 'var(--red)' : 'var(--text3)',
                    ],
                    ['Projects', sel.projects.length, 'var(--accent)'],
                  ].map(([k, v, c]) => (
                    <div
                      key={k}
                      style={{
                ...
                {sel.outstanding > 0 && (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, color: 'var(--red)' }}>
                      {fmt(sel.outstanding)} pending. Follow up required.
                    </span>
                  </div>
                )}
              </div>
            ...
            {isAdmin && (
              <div className="ps">
                ...
                >
                  Invoices ({sel.invoices.length})
                </div>
                {sel.invoices.map((inv) => {
                  const rec = totalRec(inv);
                  return (
                    <div
                      key={inv.id}
                    ...
                    >
                      ...
                      {rec > 0 && (
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text3)' }}>
                            {fmt(rec)} received · {fmt(inv.amount - rec)}{' '}
                            outstanding
                          </div>
                        </div>
                      )}
...
function CrewView({ allCrew, setAllCrew, projects, role }) {
  ...
  const fset = (k) => (v) => setForm((f) => ({ ...f, [k]: v }));
  const shown = allCrew.filter(
    (c) =>
      (filterR === 'All' || c.role === filterR) &&
      (c.name.toLowerCase().includes(search.toLowerCase()) ||
        c.role.toLowerCase().includes(search.toLowerCase()))
  );
  const updM = (id, patch) => {
    if (!isAdmin) return;
    setAllCrew((c) => c.map((m) => (m.id === id ? { ...m, ...patch } : m)));
  };
  const delM = (id) => {
    if (!isAdmin) return;
  ...
  const doAdd = () => {
    ...
    const tags = form.tags
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    setAllCrew((c) => [...c, { ...form, id: Date.now(), tags, projects: [] }]);
    setForm({
      name: '',
      role: 'DOP',
      phone: '',
  ...
  return (
    <div>
      ...
      >
        <SC2
          label="Total crew"
          value={allCrew.length}
          color="var(--text)"
          icon="👥"
          delay={0}
        />
        {[
          ['Directors', 'Director', 'var(--purple)', '🎥'],
          ['DOPs', 'DOP', 'var(--teal)', '📷'],
          ['Editors', 'Editor', 'var(--red)', '✂'],
        ].map(([l, r, c, ic]) => (
          <SC2
            key={l}
            label={l}
            value={allCrew.filter((m) => m.role === r).length}
            color={c}
            icon={ic}
            delay={50}
          />
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {[
            ...
            <button
              key={r}
              className={`fpill${filterR === r ? ' active' : ''}`}
              onClick={() => setFilterR(r)}
            >
              {r}
            </button>
          ))}
      ...
      >
        {shown.map((c, i) => {
          const cP = projects.filter((p) => c.projects.includes(p.id));
          return (
            <div
              key={c.id}
              className="card clickable fade-up"
            ...
            >
              ...
              >
                <div style={{ fontSize: 12, color: 'var(--text3)' }}>
                  {cP.length} project{cP.length !== 1 ? 's' : ''}
                </div>
                <div
                  style={{
                    fontSize: 11,
      ...
      {selected &&
        (() => {
          const m = allCrew.find((x) => x.id === selected);
          if (!m) return null;
          const cP = projects.filter((p) => m.projects.includes(p.id));
          const idx = allCrew.findIndex((x) => x.id === selected);
          return (
            <>
              <div className="povl" onClick={() => setSelected(null)} />
              <div className="panel" style={{ width: 460 }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <Av name={m.name} idx={idx} size={46} />
                      <div>
                        <div
                          style={{
                            fontSize: 19,
                  ...
                  >
                    ...
                    <div>
                      ...
                      {isAdmin ? (
                        <Inp
                          value={m.location || ''}
                          onChange={(v) => updM(m.id, { location: v })}
                          placeholder="Mumbai"
                        />
                      ) : (
                        <div style={{ fontSize: 13, color: 'var(--text2)' }}>
...
function QuotesView({ projects }) {
  ...
  const tax = (q) => sub(q) * (q.taxPct / 100);
  const grand = (q) => sub(q) + tax(q);
  const create = () => {
    if (!form.title.trim() || !form.client.trim()) return;
    const q = {
      ...form,
      ...
      lines: [{ id: 1, desc: '', cat: 'Day Rate', qty: 1, rate: 0 }],
  ...
  const dlQ = (q) => {
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Quote — ${
      q.title
    }</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;font-size:13px;color:#000;background:#fff;}.page{max-width:800px;margin:0 auto;padding:32px;}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;border-bottom:3px solid #000;padding-bottom:20px;}.co{font-size:22px;font-weight:900;}.co-s{font-size:10px;letter-spacing:0.15em;color:#555;margin-top:3px;}.qt{font-size:28px;font-weight:900;color:#e4002b;text-align:right;}.meta{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;}.mb{font-size:12px;line-height:1.8;}.mb b{display:block;font-size:10px;letter-spacing:0.1em;text-transform:uppercase;color:#555;margin-bottom:4px;}table{width:100%;border-collapse:collapse;margin-bottom:20px;}th{background:#000;color:#fff;padding:9px 12px;font-size:11px;text-align:left;letter-spacing:0.06em;}td{padding:9px 12px;border-bottom:1px solid #ddd;font-size:12px;}.ts{border-top:2px solid #000;padding-top:12px;}.tr{display:flex;justify-content:space-between;padding:5px 0;font-size:13px;}.grand{font-weight:900;font-size:16px;border-top:2px solid #000;padding-top:8px;margin-top:8px;}.foot{margin-top:32px;font-size:11px;color:#888;border-top:1px solid #ddd;padding-top:12px;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="page"><div class="header"><div><div class="co">FRAME OS</div><div class="co-s">PRODUCTION SUITE</div></div><div><div class="qt">ESTIMATE</div><div style="font-size:11px;text-align:right;margin-top:6px;color:#555;">Quote #${q.id
      .toString()
      .slice(-6)}<br>Date: ${q.createdAt}<br>Valid until: ${
      q.validUntil || '30 days'
    }</div></div></div><div class="meta"><div class="mb"><b>Prepared for</b>${
      q.client
    }${
      q.project ? `<br>${q.project}` : ''
    }</div><div class="mb"><b>Status</b><span style="background:${
      q.status === 'Approved'
        ? '#30d158'
        : q.status === 'Sent'
        ? '#3a8ef6'
        : '#666'
    };color:#fff;padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;">${
      q.status
    }</span></div></div><table><thead><tr><th style="width:40%">Description</th><th>Category</th><th style="text-align:center">Qty</th><th style="text-align:right">Rate</th><th style="text-align:right">Amount</th></tr></thead><tbody>${(
      q.lines || []
    )
      .map(
        (l, i) =>
          `<tr style="${i % 2 === 1 ? 'background:#f9f9f9' : ''}"><td>${
            l.desc || '—'
          }</td><td style="color:#666">${
            l.cat
          }</td><td style="text-align:center">${
            l.qty
          }</td><td style="text-align:right">₹${Number(l.rate).toLocaleString(
            'en-IN'
      ...
      .join(
        ''
      )}</tbody></table><div class="ts"><div class="tr"><span>Subtotal</span><span>₹${sub(
      q
    ).toLocaleString('en-IN')}</span></div><div class="tr"><span>GST (${
      q.taxPct
    }%)</span><span>₹${tax(q).toLocaleString(
      'en-IN'
    )}</span></div><div class="tr grand"><span>TOTAL</span><span>₹${grand(
      q
    ).toLocaleString('en-IN')}</span></div></div>${
      q.notes
        ? `<div style="margin-top:20px;font-size:12px;background:#f9f9f9;padding:12px;border-left:3px solid #e4002b;"><b>Notes:</b> ${q.notes}</div>`
        : ''
    }<div class="foot">This is an estimate. Valid for ${
      q.validUntil || '30 days'
    } from date of issue.</div></div></body></html>`;
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([html], { type: 'text/html' }));
    a.download = `Quote_${q.title.replace(/\s+/g, '_')}.html`;
    a.click();
  };
  ...
  return (
    <div>
      ...
      >
        ...
        <SC2
          label="Total value"
          value={fmtK(quotes.reduce((s, q) => s + grand(q), 0))}
          color="var(--accent)"
          icon="₹"
          delay={150}
        />
      ...
      {quotes.length === 0 && (
        ...
        >
          No quotes yet. Create your first estimate.
        </div>
      )}
      <div
        style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}
          ...
          >
            <div
              style={{
                ...
                right: 0,
                height: 2,
                background: (SC[q.status] || { dot: 'var(--text3)' }).dot,
              }}
            />
            <div
              style={{
      ...
      {selQ && (
        <>
          <div className="povl" onClick={() => setSelected(null)} />
          <div className="panel" style={{ width: 580 }}>
            <div
              style={{
                padding: '18px 24px 14px',
                borderBottom: '1px solid var(--border)',
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                {['Draft', 'Sent', 'Approved'].map((st) => {
                  const act = selQ.status === st;
                  const ss = SC[st];
                  return (
                    <button
                      key={st}
                      onClick={() => updQ(selQ.id, { status: st })}
                      style={{
                        fontSize: 11,
                        fontFamily: "'DM Mono',monospace",
                        padding: '3px 12px',
            ...
            <div className="ps">
              ...
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '2fr 1.2fr 0.5fr 0.8fr 0.8fr 28px',
                  gap: 6,
                  marginBottom: 6,
                }}
              >
                ...
                >
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      updQ(selQ.id, { lines: n });
                    }}
                    placeholder="e.g. Director day rate"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const n = [...selQ.lines];
                      n[i] = { ...n[i], qty: Number(e.target.value) || 1 };
                      updQ(selQ.id, { lines: n });
                    }}
                    style={{
                      padding: '7px 8px',
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const n = [...selQ.lines];
                      n[i] = { ...n[i], rate: Number(e.target.value) || 0 };
                      updQ(selQ.id, { lines: n });
                    }}
                  ...
                  <button
                    onClick={() =>
                      updQ(selQ.id, {
                        lines: selQ.lines.filter((_, j) => j !== i),
                      })
                    }
                    style={{
                      background: 'none',
              ...
              >
                {[
                  ['Subtotal', fmt(sub(selQ))],
                  [`GST (${selQ.taxPct}%)`, fmt(tax(selQ))],
                ].map(([k, v]) => (
                  <div
                    key={k}
                    style={{
              ...
              >
                <div>
                  ...
                  <Inp
                    type="date"
                    value={selQ.validUntil}
                    onChange={(v) => updQ(selQ.id, { validUntil: v })}
                  />
                </div>
                <div>
                  <Lbl ch="GST %" />
                  <Inp
                    type="number"
                    value={selQ.taxPct}
                    onChange={(v) => updQ(selQ.id, { taxPct: Number(v) || 0 })}
                    placeholder="18"
                  />
                </div>
              </div>
              <div style={{ marginTop: 10 }}>
                ...
                <TA
                  value={selQ.notes || ''}
                  onChange={(v) => updQ(selQ.id, { notes: v })}
                  placeholder="Payment terms, exclusions…"
                />
              </div>
            </div>
          </div>
      ...
      {showAdd && (
        <Modal title="New quote / estimate" onClose={() => setShowAdd(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
            <div>
              <Lbl ch="Title" />
              <Inp
                value={form.title}
                onChange={fset('title')}
                placeholder="Zara AW26 — TVC Estimate"
              />
            </div>
            <div
              style={{
    ...
    >
      ...
      <div
        style={{
          ...
          height: '1px',
          background:
            'linear-gradient(90deg,transparent,rgba(58,142,246,0.7),transparent)',
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            >
              ...
              />
              <span>
                <b style={{ color: 'var(--text)' }}>Admin</b> — full access,
                edit everything
              </span>
            </div>
            <div
            ...
            >
              ...
              />
              <span>
                <b style={{ color: 'var(--text)' }}>Viewer</b> — read-only, no
                financial data
              </span>
            </div>
          </div>
...
function ChangePassModal({ onClose, onSave, viewerPassword, onSaveViewer }) {
  ...
  const saveV = () => {
    if (!vcur || !vnext) {
      setVerr('Fill all fields.');
      return;
    }
    if (vnext !== vconf) {
      setVerr("Passwords don't match.");
      return;
    }
    if (vnext.length < 4) {
      setVerr('Min 4 characters.');
      return;
    }
    onSaveViewer(vcur, vnext);
    setVerr('');
  };
  return (
    <Modal title="Manage passwords" onClose={onClose} width={440}>
      ...
      {tab === 'admin' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          >
            Admin password unlocks full access — edit, add, delete everything.
          </div>
          <div>
            <Lbl ch="Current admin password" />
            <Inp
              type="password"
              value={cur}
              onChange={setCur}
          ...
          >
            ...
            <button className="btn-p" onClick={saveA}>
      ...
      {tab === 'viewer' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          >
            Viewer password gives read-only access — no financial data, no
            editing.
          </div>
          <div>
            <Lbl ch="Current viewer password" />
            <Inp
              type="password"
              value={vcur}
              onChange={setVcur}
          ...
          >
            ...
            <button className="btn-p" onClick={saveV}>
...
/* ── SIDEBAR ── */
const NAV_A = [
  { id: 'projects', label: 'Projects', icon: '🎬', sub: 'Shoots & pipeline' },
  { id: 'finance', label: 'Finance', icon: '₹', sub: 'Invoices & revenue' },
  { id: 'clients', label: 'Clients', icon: '🏢', sub: 'Client directory' },
  { id: 'crew', label: 'Crew', icon: '👥', sub: 'Cast & crew' },
  { id: 'quotes', label: 'Quotes', icon: '📋', sub: 'Estimates & quotes' },
];
const NAV_V = [
  { id: 'projects', label: 'Projects', icon: '🎬', sub: 'Shoots & pipeline' },
  { id: 'clients', label: 'Clients', icon: '🏢', sub: 'Client directory' },
...
function Sidebar({
  ...
  const commitName = () => {
    const t = draft.trim();
    if (t) setStudioName(t);
    else setDraft(studioName);
    setEditName(false);
  };
  return (
    <nav
          ...
          >
            <div
              style={{
                ...
                height: 28,
                borderRadius: '50%',
                background: isAdmin ? '#1d3461' : '#1a3a20',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                fontSize: 11,
                fontWeight: 700,
                color: isAdmin ? '#60a0f8' : '#4ade96',
                flexShrink: 0,
              }}
            >
              {isAdmin ? 'A' : 'V'}
...
export default function App() {
  const [role, setRole] = useState(
    () => sessionStorage.getItem('frameOS_role') || null
  );
  const [splash, setSplash] = useState(
    () => !sessionStorage.getItem('frameOS_splashDone')
  );
  ...
  const [studioName, setStudioName] = usePersist(
    'frameOS_studioName',
    'Frame OS'
  );
  ...
  const tryUnlock = (pw) => {
    if (pw === password) {
      sessionStorage.setItem('frameOS_role', 'admin');
      setRole('admin');
      return 'admin';
    }
    if (pw === viewerPw) {
      sessionStorage.setItem('frameOS_role', 'viewer');
      setRole('viewer');
      return 'viewer';
    }
    return null;
  };
  const handleChgAdmin = (cur, next) => {
    if (cur !== password) {
      alert('Current admin password incorrect.');
      return;
    }
    setPassword(next);
    setShowChgPw(false);
  };
  const handleChgViewer = (cur, next) => {
    if (cur !== viewerPw) {
      alert('Current viewer password incorrect.');
      return;
    }
    setViewerPw(next);
    setShowChgPw(false);
  ...
  const safeTab =
    isViewer && !NAV_V.find((n) => n.id === tab) ? 'projects' : tab;
  ...
  if (splash)
    return (
      <>
        ...
        <SplashScreen
          onDone={() => {
            sessionStorage.setItem('frameOS_splashDone', '1');
            setSplash(false);
          }}
  ...
  if (!role)
    return (
      <>
        ...
        <LockScreen onUnlock={tryUnlock} studioName={studioName} />
      </>
    );
  return (
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            flexDirection: 'column',
            minHeight: '100vh',
            transition: 'margin-left .25s cubic-bezier(.32,.72,0,1)',
          }}
        >
          <header
            style={{
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {isAdmin && (
                ...
                >
                  🔒 <span className="hmob">Passwords</span>
                </button>
              )}
              <button
                onClick={() => {
                  sessionStorage.removeItem('frameOS_role');
                  setRole(null);
                }}
                style={{
                  background: 'var(--bg4)',
          ...
          <main style={{ flex: 1, padding: '26px 28px' }}>
            {safeTab === 'projects' && (
              <ProjectsView
                invoices={LS.get('frameOS_invoices', SEED_INV)}
                allCrew={allCrew}
                setAllCrew={setAllCrew}
                role={role}
                expTrackerUrl={expUrl}
              />
            )}
            ...
            {safeTab === 'quotes' && isAdmin && (
              <QuotesView projects={projects} />
            )}
          </main>
          {/* Mobile bottom nav */}
          <nav className="bnav">
            {NAV.map((n) => (
              <button
                key={n.id}
                className={`bni${safeTab === n.id ? ' active' : ''}`}
                onClick={() => setTab(n.id)}
              >
          ...
          >
            ...
            >
              {studioName} · v2.1 ·{' '}
              {isAdmin ? 'Data saved locally' : 'View only'}
            </span>
            {isAdmin && (
              <span
                style={{
      ...
      {showChgPw && (
        <ChangePassModal
          ...
          onSave={handleChgAdmin}
          viewerPassword={viewerPw}
          onSaveViewer={handleChgViewer}
        />
      )}
      {showExpUrl && (
        <Modal
          title="Link Expense Tracker"
          onClose={() => setShowExpUrl(false)}
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            >
              Paste the URL of your Expense Tracker HTML file (hosted on Netlify
              or opened locally). This enables the 💰 Expenses button in the
              header and auto-matches project expenses by client name.
            </div>
            <div>
              <Lbl ch="Expense Tracker URL" />
              <Inp
                value={expUrl}
                onChange={setExpUrl}
                placeholder="https://your-tracker.netlify.app"
              />
            </div>
            <div
              style={{
            ...
            >
              <button className="btn-g" onClick={() => setShowExpUrl(false)}>
                Cancel
              </button>
              <button className="btn-p" onClick={() => setShowExpUrl(false)}>
                Save
              </button>
            </div>
          </div>
