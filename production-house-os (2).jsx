...
function StCell({status,onChange}){const[open,setOpen]=useState(false);const ref=useRef();useEffect(()=>{if(!open)return;const h=e=>{if(ref.current&&!ref.current.contains(e.target))setOpen(false);};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h);},[open]);return <div ref={ref} style={{padding:"10px 16px",position:"relative"}}><div onClick={()=>setOpen(o=>!o)} style={{display:"inline-flex",alignItems:"center",gap:5,cursor:"pointer"}}><Badge status={status}/><span style={{fontSize:10,color:"var(--text3)"}}>▾</span></div>{open&&<div style={{position:"absolute",top:"calc(100% - 4px)",left:16,zIndex:90,background:"#1e1e24",border:"1px solid var(--border2)",borderRadius:10,padding:6,minWidth:130,boxShadow:"0 12px 36px rgba(0,0,0,.55)"}}>{INV_ST.map(st=>{const ss=SC[st];const act=status===st;return <div key={st} onClick={()=>{onChange(st);setOpen(false);}} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:7,cursor:"pointer",background:act?"var(--bg4)":"transparent"}} onMouseEnter={e=>e.currentTarget.style.background="var(--bg4)"} onMouseLeave={e=>e.currentTarget.style.background=act?"var(--bg4)":"transparent"}><span style={{width:7,height:7,borderRadius:"50%",background:ss.dot,display:"inline-block",flexShrink:0}}/><span style={{fontSize:12,fontFamily:"'DM Mono',monospace",color:act?ss.dot:"var(--text2)",fontWeight:act?500:400}}>{st}</span>{act&&<span style={{marginLeft:"auto",fontSize:11,color:ss.dot}}>✓</span>}</div>;})}</div>}</div>;}
      ...
      <div className="mc" style={{flex:1,marginLeft:sideW,display:"flex",flexDirection:"column",minHeight:"100vh",transition:"margin-left .25s cubic-bezier(.32,.72,0,1)"}}>
        <header style={{position:"sticky",top:0,zIndex:40,height:"var(--header-h)",background:"rgba(14,14,16,0.9)",backdropFilter:"blur(18px)",WebkitBackdropFilter:"blur(18px)",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:14}} className="hpad" id="hdr">
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            {isAdmin&&expUrl&&<a href={expUrl} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:6,background:"var(--amber-bg)",border:"1px solid var(--amber)44",borderRadius:9,padding:"6px 12px",cursor:"pointer",fontSize:13,fontWeight:500,color:"var(--amber)",textDecoration:"none",whiteSpace:"nowrap"}}>💰 Expenses</a>}
            {isAdmin&&<button onClick={()=>setShowExpUrl(true)} className="hmob" style={{background:"var(--bg4)",border:"1px solid var(--border)",borderRadius:9,padding:"7px 10px",cursor:"pointer",color:"var(--text2)",fontSize:12,fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap"}} title="Set expense tracker URL">🔗 Exp. Tracker</button>}
            {isAdmin&&<button onClick={()=>setShowChgPw(true)} style={{background:"var(--bg4)",border:"1px solid var(--border)",borderRadius:9,padding:"7px 12px",cursor:"pointer",color:"var(--text2)",fontSize:13,display:"flex",alignItems:"center",gap:5,fontFamily:"'DM Sans',sans-serif",fontWeight:500,whiteSpace:"nowrap"}}>🔒 <span className="hmob">Passwords</span></button>}
            <button onClick={()=>{sessionStorage.removeItem("frameOS_role");setRole(null);}} style={{background:"var(--bg4)",border:"1px solid var(--border)",borderRadius:9,padding:"7px 12px",cursor:"pointer",color:"var(--text2)",fontSize:13,display:"flex",alignItems:"center",gap:5,fontFamily:"'DM Sans',sans-serif",fontWeight:500,whiteSpace:"nowrap"}}>↩ <span className="hmob">Lock</span></button>
          </div>
        </header>
